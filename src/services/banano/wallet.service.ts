...
import {IWalletAccount,IFullWallet,IWalletApiAccount} from "./models"
...
export class WalletService {
    ...
    this.websocket.newTransactions$.subscribe(async (transaction) => {
      ...
      if (transaction.block.type == 'send' && walletAccountIDs.indexOf(transaction.block.destination) !== -1) {
        ...
        if (walletAccount) {
          ...
          console.log("<--- websocket new transaction["+transaction+"]");
        ...
        console.log("tttttt-> websocket new transaction:["+transaction+"] processStateBlock");
  ...
  async createSeedAccount(index) {
    ...
    const newAccount: IWalletAccount = {
      id: accountName,
      frontier: null,
      secret: accountBytes,
  ...
  async addWalletAccount(accountIndex: number|null = null, reloadBalances: boolean = true) {
    ...
    let newAccount: IWalletAccount|null;
    ...
    if (this.wallet.type === 'privateKey') {
      ...
      try {
        throw new Error("ledger type is not supported");
      } catch (err) {
  ...
  async getAccountsDetails(): Promise<IWalletApiAccount[]> {
